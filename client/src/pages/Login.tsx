...
export default function Login() {
  ...
  const loginMutation = trpc.auth.login.useMutation({
    ...
    onError: (err) => toast.error(err.message || "بيانات الدخول غير صحيحة"),
  });
      ...
      <Card className={`w-full max-w-md relative z-10 backdrop-blur-3xl transition-all duration-1000 shadow-2xl rounded-[3rem] p-10 overflow-hidden ${
        ...
        <div className="text-center mb-8">
          ...
          <div className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center shadow-2xl border transition-all transform rotate-3 ${
              isFemale ? 'bg-gradient-to-tr from-pink-500 to-rose-600 border-pink-400 text-white' : 'bg-gradient-to-tr from-blue-600 to-indigo-700 border-blue-400/30 text-white shadow-blue-500/20'
          }`}>
            {isFemale ? <Sparkles size={32} /> : <ShieldCheck size={32} />}
          </div>
          <h1 className={`text-2xl font-bold mb-1 glow-text ${isFemale ? 'text-pink-600' : 'text-blue-900 dark:text-white'}`}>{t("login")}</h1>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center px-4">
              <label className={`text-[10px] font-black uppercase tracking-widest glow-text ${isFemale ? 'text-pink-400' : 'text-blue-500 dark:text-blue-400'}`}>{t("password")}</label>
            </div>
            ...
            <div className="my-4 text-center">
              <button type="button" onClick={() => navigate("/forgot-password")} className={`text-xs font-bold hover:underline transition-colors glow-text ${isFemale ? 'text-pink-500' : 'text-blue-600 dark:text-blue-400'}`}>نسيت كلمة المرور؟</button>
            </div>
          </div>

          <Button type="submit" disabled={loginMutation.isPending} className={`w-full h-14 font-black text-lg rounded-2xl shadow-xl transition-all hover:scale-[1.02] active:scale-95 glow-text ${
            isFemale ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white' : 'bg-gradient-to-r from-blue-600 to-indigo-700 text-white'
          }`}>
        ...
        <div className="text-center pt-8 border-t border-border/30 mt-6">
          ...
          <p className={`text-sm mb-4 font-medium ${isFemale ? 'text-pink-900' : 'text-blue-900 dark:text-blue-100'}`}>
            {t("dont_have_account")}
          </p>
          <Button 
            variant="link" 
            onClick={() => navigate("/register")} 
            className={`font-bold hover:underline p-0 h-auto ${isFemale ? 'text-pink-600' : 'text-blue-600 dark:text-blue-400'}`}
          >
            {t("register")}
          </Button>
        </div>
